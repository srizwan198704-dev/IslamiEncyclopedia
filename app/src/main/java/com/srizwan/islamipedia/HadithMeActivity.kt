...
data class HadithItem(
    ...
    val bookInnerTitle: String = "" // সহীহ বুখারী
)
...
class HadithMeActivity : AppCompatActivity() {
    ...
    private fun buildUI(): View {
        ...
        offlineIndicator = TextView(this).apply {
            text = "⚠ অফলাইন মোড - ক্যাশে করা ডেটা দেখানো হচ্ছে"
            textSize = 12f
            setTextColor(Color.WHITE)
            typeface = getBengaliTypeface()
            gravity = Gravity.CENTER
        ...
        statusProgressBar = ProgressBar(this, null, android.R.attr.progressBarStyleLarge).apply {
            ...
            layoutParams = LinearLayout.LayoutParams(dp(56), dp(56)).apply { bottomMargin = dp(16); gravity = Gravity.CENTER_HORIZONTAL }
            visibility = View.GONE
        }
        ...
        fabSearchBtn = FrameLayout(this).apply {
            val size = dp(52)
            layoutParams = FrameLayout.LayoutParams(size, size, Gravity.BOTTOM or Gravity.END).apply { setMargins(0, 0, dp(20), dp(20)) }
    ...
    private fun buildGlobalSearchOverlay(): FrameLayout {
        ...
        header.addView(TextView(this).apply {
            ...
            setTextColor(Color.WHITE)
            typeface = getBengaliTypeface()
            layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).apply { setMargins(dp(10), 0, 0, 0) }
        })
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (cached!= null) {
                    ...
                    withContext(Dispatchers.Main) {
                        offlineIndicator.visibility = View.VISIBLE
                        offlineIndicator.text = "⚠ নেটওয়ার্ক নেই এবং ক্যাশে ডেটা পাওয়া যায়নি"
                        isNetworkAvailable = false
                    }
                    throw e
                }
    ...
    private fun loadHadith(bookId: Int, sectionId: Int, bookTitle: String, sectionTitle: String) {
        ...
        currentRequestJob = scope.launch {
            try {
                ...
                if (gen!= loadGeneration) return@launch
                showError("হাদিস লোড করতে সমস্যা হয়েছে") { loadHadith(bookId, sectionId, bookTitle, sectionTitle) }
            }
        }
    }

    // FINAL FIX for your JSON - book is object, not array
        ...
        return (0 until arr.length()).map { arr.getJSONObject(it) }.map { o ->
            ...
            try {
                if (o.has("book")) {
                    ...
                    if (bookObj!= null) {
                        innerBookTitle = bookObj.safeString("title",
                            bookObj.safeString("title_en",
                                bookObj.safeString("title_ar", "")))
                    } else {
                        val bookArr = o.optJSONArray("book")
                        if (bookArr!= null && bookArr.length() > 0) {
                            val first = bookArr.getJSONObject(0)
                            innerBookTitle = first.safeString("title",
                                first.safeString("title_en", ""))
                        }
                    }
                }
                if (innerBookTitle.isBlank() && o.has("chapter")) {
                    val chapterObj = o.optJSONObject("chapter")
                    if (chapterObj!= null) {
                        // যদি book খালি থাকে chapter title fallback
                        // innerBookTitle = chapterObj.safeString("title","")
                    }
                }
            } catch (e: Exception) { innerBookTitle = "" }

            HadithItem(
                hadithNumber = o.optInt("hadith_number", o.optInt("hadith_number_one", 0)),
                title = o.safeString("title"),
                descriptionAr = o.safeString("description_ar"),
                description = o.safeString("description"),
                bookInnerTitle = innerBookTitle // এখানে "সহীহ বুখারী" আসবে
            )
        ...
        val job = scope.launch {
            try {
                ...
                for (section in sections) {
                    ...
                    HadithCache.hadith["${book.id}_${section.id}"] = list
                    done++
    ...
    private fun performSearch(query: String) {
        ...
        when (val s = currentState) {
            ...
            is PageState.Sections -> {
                ...
                recyclerView.adapter = SectionAdapter(filteredSections) { section -> saveScrollPosition(); loadHadith(s.bookId, section.id, s.bookTitle, section.title) }
                showContent()
            }
            is PageState.Hadith -> {
                filteredHadith = currentHadithList.filter { h -> h.hadithNumber.toString().contains(term) || h.title.stripHtml().lowercase().contains(term) || h.description.stripHtml().lowercase().contains(term) || h.bookInnerTitle.lowercase().contains(term) }
                recyclerView.adapter = HadithAdapter(filteredHadith, s.bookTitle, onCopy = { h -> copyHadith(h, s.bookTitle, s.sectionTitle) }, onShare = { h -> shareHadith(h, s.bookTitle, s.sectionTitle) })
                showContent()
            }
        }
    }
    ...
    private fun performGlobalSearchFromDownloaded(query: String) {
        if (downloadedBookIds.isEmpty()) {
            globalSearchStatus.text = "⚠ কোনো বই ডাউনলোড করা হয়নি।"
        ...
        scope.launch(Dispatchers.Default) {
            ...
            for (book in targetBooks) {
                ...
                val sections = HadithCache.sections[book.id]?: run {
                    ...
                    if (cachedSectionsJson!= null) parseSections(cachedSectionsJson) else emptyList()
                }
                for (section in sections) {
                    if (gen!= globalSearchGeneration) return@launch
                    ...
                    val hadithList = HadithCache.hadith[k]?: run {
                        ...
                        if (cachedHadithJson!= null) parseHadith(cachedHadithJson) else emptyList()
                    }
                    if (hadithList.isEmpty()) continue
                    totalHadith += hadithList.size
                    hadithList.filter { h -> h.hadithNumber.toString().contains(term) || h.title.stripHtml().lowercase().contains(term) || h.description.stripHtml().lowercase().contains(term) || h.bookInnerTitle.lowercase().contains(term) }
    ...
    private fun buildDownloadControl(book: BookItem): View {
        ...
        when {
            ...
            downloadingBookIds.contains(book.id) -> {
                ...
                box.background = createRoundedBg(Color.parseColor("#E8F8F7"), Color.parseColor("#01837A"), dp(1), size / 2)
                ...
                box.visibility = View.VISIBLE
                box.background = createRoundedBg(Color.WHITE, Color.parseColor("#01837A"), dp(1), size / 2)
                box.addView(TextView(this@HadithMeActivity).apply {
                    text = "⬇"; textSize = 15f; setTextColor(Color.parseColor("#01837A")); gravity = Gravity.CENTER
                    layoutParams = FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT)
                })
    ...
    inner class SkeletonAdapter(private val type: SkeletonType) : RecyclerView.Adapter<SkeletonAdapter.VH>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
            val card = LinearLayout(this@HadithMeActivity).apply {
                orientation = LinearLayout.VERTICAL
                layoutParams = RecyclerView.LayoutParams(RecyclerView.LayoutParams.MATCH_PARENT, RecyclerView.LayoutParams.WRAP_CONTENT).apply { bottomMargin = dp(14) }
            ...
            headerRow.addView(bar(150, 18, 0).apply { (layoutParams as LinearLayout.LayoutParams).marginStart = dp(10) })
    ...
    inner class BookAdapter(private val items: List<BookItem>, private val onClick: (BookItem) -> Unit) : RecyclerView.Adapter<BookAdapter.VH>() {
        ...
        override fun onBindViewHolder(holder: VH, position: Int) {
            ...
            headerRow.addView(TextView(this@HadithMeActivity).apply {
                text = toBangla(book.originalPosition + 1); textSize = 13f; setTextColor(Color.WHITE); typeface = getBengaliTypeface(); background = createRoundedSolid(Color.parseColor("#01837A"), dp(16)); gravity = Gravity.CENTER; setPadding(dp(10), dp(4), dp(10), dp(4))
            ...
            if (displayTitle.isNotBlank()) {
                headerRow.addView(TextView(this@HadithMeActivity).apply { text = displayTitle; textSize = 17f; setTextColor(Color.parseColor("#01837A")); typeface = getBengaliTypeface(); layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f) })
            }
    ...
    inner class SectionAdapter(private val items: List<SectionItem>, private val onClick: (SectionItem) -> Unit) : RecyclerView.Adapter<SectionAdapter.VH>() {
        ...
        override fun onBindViewHolder(holder: VH, position: Int) {
            ...
            holder.card.addView(TextView(this@HadithMeActivity).apply { text = section.title; textSize = 17f; setTextColor(Color.parseColor("#01837A")); typeface = getBengaliTypeface() })
        }
        override fun getItemCount() = items.size
    }
    ...
    inner class HadithAdapter(
        ...
        override fun onBindViewHolder(holder: VH, position: Int) {
            ...
            val numberText = if (hadith.bookInnerTitle.isNotBlank()) {
                "হাদিস নং: ${toBangla(hadith.hadithNumber)} • ${hadith.bookInnerTitle}"
            } else {
            ...
            headerRow.addView(ImageView(this@HadithMeActivity).apply {
                ...
                layoutParams = LinearLayout.LayoutParams(dp(24), dp(24)).apply { marginStart = dp(8); marginEnd = dp(10) }
            ...
            if (hadith.title.trim().isNotBlank()) {
                holder.card.addView(TextView(this@HadithMeActivity).apply {
                    textSize = 18f; setTextColor(Color.parseColor("#01837A")); typeface = getBengaliTypeface()
                    ...
                    setSmartText(hadith.title.trim())
                })
            }
            if (hadith.descriptionAr.trim().isNotBlank()) {
                holder.card.addView(TextView(this@HadithMeActivity).apply {
                    textSize = 20f; setTextColor(Color.parseColor("#333333")); typeface = getArabicTypeface(); gravity = Gravity.END
                    ...
                    setSmartText(hadith.descriptionAr.trim())
                })
            }
            if (hadith.description.trim().isNotBlank()) {
                holder.card.addView(TextView(this@HadithMeActivity).apply {
                    textSize = 18f; setTextColor(Color.parseColor("#444444")); typeface = getBengaliTypeface()
                    ...
                    setSmartText(hadith.description.trim())
                })
            }
        }
        override fun getItemCount() = items.size
    ...
    inner class GlobalSearchAdapter(
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = VH(LinearLayout(this@HadithMeActivity).apply {
            orientation = LinearLayout.VERTICAL
            layoutParams = RecyclerView.LayoutParams(RecyclerView.LayoutParams.MATCH_PARENT, RecyclerView.LayoutParams.WRAP_CONTENT).apply { bottomMargin = dp(12) }
        ...
        override fun onBindViewHolder(holder: VH, position: Int) {
            ...
            holder.card.removeAllViews()
            holder.card.addView(TextView(this@HadithMeActivity).apply {
                text = "হাদিস নং: ${toBangla(result.hadith.hadithNumber)} • ${result.hadith.bookInnerTitle.ifBlank { result.bookTitle }}"
                textSize = 12f; setTextColor(Color.WHITE); typeface = getBengaliTypeface(); background = createRoundedSolid(Color.parseColor("#01837A"), dp(20)); setPadding(dp(10), dp(4), dp(10), dp(4))
            })
        }
        override fun getItemCount() = items.size
    }
