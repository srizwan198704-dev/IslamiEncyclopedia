...
public class BookonlinefullviewActivity extends AppCompatActivity {
		...
		if (FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২")))) {
			...
			if (!FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২")))) {
				...
				if (Rizwan.isConnected(getApplicationContext())) {
					...
					if (FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২"))) {
						spin.setVisibility(View.GONE);
						content.setVisibility(View.VISIBLE);
						Nointernet.setVisibility(View.GONE);
					}
	...
	public void _json_search(final String _charSeq) {
		...
		for(int _repeat64 = 0; _repeat64 < (int)(length); _repeat64++) {
			value1 = chapter.get((int)r).get("1").toString();
	...
	public class ListView1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			if (chapter.get((int)_position).containsKey("1")) {
				name.setText(chapter.get((int)_position).get("1").toString());
			...
			linear1.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View _view) {
					// Check if the author (chapter.get("2")) is empty
					if (chapter.get((int)_position).get("2").toString().equals("")) {
						...
						editor.putString("name", chapter.get((int)_position).get("1").toString());
						editor.putString("author", chapter.get((int)_position).get("2").toString());
						editor.apply(); // Apply changes

						// Optionally, show a toast message indicating that the data has been saved
						//Toast.makeText(getApplicationContext(), "বই তথ্য সংরক্ষণ করা হয়েছে", Toast.LENGTH_SHORT).show();

						// Pass the data (name and author) to ViewActivity via Intent
