...
public class BookonlineActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		refresh.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (Rizwan.isConnected(getApplicationContext())) {
					if (FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/")))) {
						refresh.setVisibility(View.GONE);
						refreshprogress.setVisibility(View.VISIBLE);
						FileUtil.deleteFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/")));
						timer = new TimerTask() {
							@Override
							public void run() {
		...
		materialbutton1.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				book.startRequestNetwork(RequestNetworkController.GET, BuildConfig.text, "", _book_request_listener);
		...
		_book_request_listener = new RequestNetwork.RequestListener() {
			...
			public void onResponse(String _param1, String _param2, HashMap<String, Object> _param3) {
				...
				if (FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২")))) {
					map = new Gson().fromJson(FileUtil.readFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২"))), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
					...
					if (!FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২")))) {
						...
						((BaseAdapter)ListView1.getAdapter()).notifyDataSetChanged();
						getsearch = new Gson().toJson(map);
						FileUtil.writeFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/".concat(bookname.getText().toString()))), getsearch);
						refresh.setVisibility(View.VISIBLE);
						refreshprogress.setVisibility(View.GONE);
						version.setText(map.get((int)0).get("version").toString());
					}
		...
		_updateBook_request_listener = new RequestNetwork.RequestListener() {
			...
			public void onResponse(String _param1, String _param2, HashMap<String, Object> _param3) {
				...
				if (Double.parseDouble(version.getText().toString()) < Double.parseDouble(updatebook.get((int)0).get("version").toString())) {
					if(!BookonlineActivity.this.isFinishing()) {
						final AlertDialog dialog1 = new AlertDialog.Builder(BookonlineActivity.this).create();
	...
	private void initializeLogic() {
		...
		bookname.setText("অনলাইন বই সমাহার ২");
		_marquue(bookname, "অনলাইন বই সমাহার ২");
		click = 0;
		...
		if (FileUtil.isExistFile(FileUtil.getPackageDataDir(getApplicationContext()).concat("/".concat("/ইসলামী বিশ্বকোষ/.অনলাইন বই ২/অনলাইন বই সমাহার ২")))) {
			...
			version.setText(map.get((int)0).get("version").toString());
			if (Rizwan.isConnected(getApplicationContext())) {
				updateBook.startRequestNetwork(RequestNetworkController.GET, "https://www.dropbox.com/scl/fi/ktymvqjj4qvd0naolx2vn/bookupdate.json?rlkey=j5e61ce0hjdy2a3dki8t8z8do&st=wt3xfpav&dl=1", "Rizwan", _updateBook_request_listener);
			} else {
			...
			if (Rizwan.isConnected(getApplicationContext())) {
				book.startRequestNetwork(RequestNetworkController.GET, BuildConfig.text, "Rizwan", _book_request_listener);
			} else {
			...
			linear1.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View _view) {
					if (map.get((int)_position).get("name").toString().equals("none")) {
						...
						in.setClass(getApplicationContext(), BookonlinefullviewActivity.class);
