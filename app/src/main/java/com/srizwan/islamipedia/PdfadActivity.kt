...
class PdfadActivity : AppCompatActivity() {
    ...
    private fun initializeLogic() {
        ...
        if (!file.exists()) {
            Toast.makeText(this, "File not found: $pdfName", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        ...
        if (lastPage > 0) {
            AlertDialog.Builder(this)
                ...
                .setCancelable(false)
                .setPositiveButton("হ্যাঁ") { _, _ -> openPdfAtPage(file, lastPage) }
                .setNegativeButton("না") { _, _ -> openPdfAtPage(file, 0) }
                .create()
                .show()
        } else {
            openPdfAtPage(file, 0)
        }
    }

    private fun openPdfAtPage(file: File, page: Int) {
    ...
    private fun captureAndSaveScreenshot(): String? {
        return try {
            // পুরনো সব screenshot মুছে দাও
            cacheDir.listFiles { file -> file.name.startsWith("screenshot_") }
                ?.forEach { it.delete() }
