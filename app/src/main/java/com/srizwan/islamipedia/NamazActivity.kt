...
class NamazActivity : AppCompatActivity() {
    ...
    // Fonts from internet - website er moto, no res, sob net theke save
    ...
    private val FONT_ANEK_URL = "https://cdn.jsdelivr.net/gh/google/fonts@main/ofl/anekbangla/AnekBangla-Regular.ttf"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(buildUiProgrammatically())
        loadFontsFromInternet() // website er moto net theke font
        loadCities()
        ...
        return try {
            ...
            if(!file.exists() || file.length()<1000){
                ...
                if(conn.responseCode==200){
                    conn.inputStream.use { input -> FileOutputStream(file).use { out -> input.copyTo(out) } }
                }
            }
    ...
    private fun applyFontsToAll(){
        ...
        try {
            fun apply(v: View){
                ...
                } else if(v is ViewGroup){ for(i in 0 until v.childCount) apply(v.getChildAt(i)) }
            }
            apply(rootContent)
            hijriTv.typeface = tfHindBold
            bengaliTv.typeface = tfHind
            cityTv.typeface = tfHindBold
        } catch(e:Exception){}
    }

    private fun buildUiProgrammatically(): View {
        val outer = LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; setBackgroundColor(Color.parseColor("#FDFBF6")) }
        val scroll = ScrollView(this).apply { layoutParams=LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 0, 1f) }
        rootContent = LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; setPadding(dp(14),dp(14),dp(14),dp(100)) }
        val header = LinearLayout(this).apply {
            ...
            background=gd; setPadding(dp(16),dp(16),dp(16),dp(20))
        }
        val topRow=LinearLayout(this).apply { orientation=LinearLayout.HORIZONTAL; gravity=Gravity.CENTER_VERTICAL }
        val brand=LinearLayout(this).apply { orientation=LinearLayout.VERTICAL }
        val appName=TextView(this).apply { text="ইসলামী বিশ্বকোষ ও আল হাদিস S2"; setTextColor(Color.WHITE); setTextSize(TypedValue.COMPLEX_UNIT_SP,15f); typeface=Typeface.DEFAULT_BOLD }
        val appSub=TextView(this).apply { text="নামাজের সময়সূচি • Masjid Edition"; setTextColor(Color.parseColor("#B0C4B8")); setTextSize(TypedValue.COMPLEX_UNIT_SP,11f) }
        ...
        hijriTv=TextView(this).apply { text="লোড হচ্ছে..."; setTextColor(Color.parseColor("#FFF6C8")); setTextSize(TypedValue.COMPLEX_UNIT_SP,15f); typeface=Typeface.DEFAULT_BOLD; gravity=Gravity.CENTER; setPadding(0,dp(18),0,0) }
        bengaliTv=TextView(this).apply { setTextColor(Color.parseColor("#C8DDD6")); setTextSize(TypedValue.COMPLEX_UNIT_SP,12f); gravity=Gravity.CENTER }
        val mosqueLine=TextView(this).apply { text="▐  ⌒  ▐  |  ⌒  |  ▐  ⌒  ▐"; setTextColor(Color.parseColor("#C9A227")); gravity=Gravity.CENTER; setTextSize(TypedValue.COMPLEX_UNIT_SP,18f); setPadding(0,dp(12),0,0) }
        val title=TextView(this).apply { text="নামাজের সময়সূচি"; setTextColor(Color.WHITE); setTextSize(TypedValue.COMPLEX_UNIT_SP,28f); typeface=Typeface.DEFAULT_BOLD; gravity=Gravity.CENTER; setPadding(0,dp(8),0,0) }
        val sunRow=LinearLayout(this).apply { orientation=LinearLayout.HORIZONTAL; gravity=Gravity.CENTER }
        sunriseTv=TextView(this).apply { text="সূর্যোদয় --"; setTextColor(Color.parseColor("#FFF6C8")); setTextSize(TypedValue.COMPLEX_UNIT_SP,11f) }
        sunsetTv=TextView(this).apply { text="সূর্যাস্ত --"; setTextColor(Color.parseColor("#FFF6C8")); setTextSize(TypedValue.COMPLEX_UNIT_SP,11f); setPadding(dp(20),0,0,0) }
        ...
        forbiddenContainer=LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; setPadding(dp(8),dp(8),dp(8),dp(8)) }; forbCard.addView(forbiddenContainer)
        val naflCard=createCard(); naflCard.addView(createCardHead("নফল নামাজের সময়সূচী", null).first)
        naflContainer=LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; setPadding(dp(8),dp(8),dp(8),dp(8)) }; naflCard.addView(naflContainer)
        sideRow.addView(forbCard); sideRow.addView(View(this).apply { layoutParams=LinearLayout.LayoutParams(-1,dp(12)) }); sideRow.addView(naflCard)
        ...
        val badgeRow=LinearLayout(this).apply { addView(trackerMonthTv, LinearLayout.LayoutParams(-2,-2).apply{setMargins(0,0,dp(6),0)}); addView(trackerCountTv) }
        ...
        trackerCard.addView(HorizontalScrollView(this).apply { addView(summaryContainer) })
            ...
            val chip=TextView(this).apply {
                ...
                setOnClickListener {
                    ...
                    filterRow.children.forEach { c-> (c as TextView).apply {
                        ...
                        if(tag==filterWaqt){ b.setColor(Color.parseColor("#0E3B2E")); setTextColor(Color.WHITE) } else { b.setColor(Color.WHITE); setTextColor(Color.BLACK) }
                    } }
                    renderTracker()
                }
            }
        ...
        countIcon=TextView(this).apply { text="🕌"; setTextSize(TypedValue.COMPLEX_UNIT_SP,18f); val bg=GradientDrawable(); bg.setColor(Color.parseColor("#FFF1A0")); bg.cornerRadius=dp(12).toFloat(); background=bg; setPadding(dp(10),dp(8),dp(10),dp(8)) }
    ...
    private fun showCityDialog(){
        ...
        val title=TextView(this).apply { text="স্থান শনাক্ত করুন"; setTextSize(TypedValue.COMPLEX_UNIT_SP,17f); typeface=tfAnekBold ?: Typeface.DEFAULT_BOLD }
        val sub=TextView(this).apply { text="নামাজের সময় এবং চাঁদের তারিখ দেখাতে আপনার অবস্থান জানা জরুরি।"; setTextSize(TypedValue.COMPLEX_UNIT_SP,12f); setTextColor(Color.GRAY); typeface=tfHind }
        val searchEt=EditText(this).apply {
            hint="শহর নির্বাচন করুন - যেমন ঢাকা"; setPadding(dp(12),dp(10),dp(12),dp(10)); typeface=tfHind
